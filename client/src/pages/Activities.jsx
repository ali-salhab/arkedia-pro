...
export default function ActivitiesPage() {
  ...
  const resolveLinkedAdmin = (a) => {
    ...
    const id = typeof a.adminCompany === "object" ? a.adminCompany._id : a.adminCompany;
  ...
  useEffect(() => {
    if (resumeCreation && adminsList.length > 0) {
      ...
      navigate("/activities/new", { state: { backTo: "/activities" } });
    }
  ...
  const handleDelete = async (id) => { if (window.confirm(t("confirmDeleteActivity"))) await deleteUser(id); };
  ...
  if (error) return <div className="card p-6 text-center text-rose-500">{t("errorLoadingActivities")}</div>;
  ...
  const exportCsv = () => {
    const csv = ["Name,Email,Linked Admin,Created", ...rows.map((a) => { const admin = resolveLinkedAdmin(a); return `"${a.name}","${a.email}","${admin ? admin.name : ""}","${a.createdAt ? new Date(a.createdAt).toLocaleDateString() : ""}"` })] .join("\n");
    const url = URL.createObjectURL(new Blob([csv], { type: "text/csv" }));
    const anchor = document.createElement("a"); anchor.href = url; anchor.download = "activities.csv"; anchor.click();
  };
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <nav className="flex items-center gap-1.5 text-sm text-slate-400 mb-1">
            <span>{t("dashboard")}</span><span>/</span><span className="text-slate-600 dark:text-slate-300">{t("allActivities")}</span>
          </nav>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">{t("allActivities")}</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{t("activitiesSubtitle") || "Manage activity accounts"}</p>
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-50 dark:divide-slate-700/60">
            {rows.length === 0 ? (
              ...
              return (
                <tr key={a._id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      {a.logo ? <img src={a.logo} className="h-9 w-9 rounded-full object-cover shrink-0" alt="" /> : <div className="h-9 w-9 rounded-full shrink-0 grid place-items-center text-white text-xs font-bold" style={{ background: bg }}>{(a.name || "?")[0].toUpperCase()}</div>}
                      <div><div className="font-medium text-slate-800 dark:text-slate-100">{a.name}</div><div className="text-xs text-slate-400 dark:text-slate-500">{a.email}</div></div>
                    </div>
                  </td>
                  <td className="px-5 py-3.5">
                    {admin ? (
                  ...
                  <td className="px-5 py-3.5 text-slate-500 dark:text-slate-400">{(a.permissions || []).length} {t("permissionsSelected")}</td>
                  <td className="px-5 py-3.5 text-slate-500 dark:text-slate-400">{a.createdAt ? new Date(a.createdAt).toLocaleDateString(lang === "ar" ? "ar-EG" : "en-US") : "—"}</td>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => handleEdit(a)} className="p-1.5 rounded-lg text-slate-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-500/10 transition"><Pencil size={15} /></button>
                      <button onClick={() => handleDelete(a._id)} className="p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition"><Trash2 size={15} /></button>
                    </div>
                  </td>
                </tr>
              );
