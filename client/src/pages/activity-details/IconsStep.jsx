...
const COPY = {
  en: {
    ...
    subtitle:
      "Select the features your activity offers. Can't find one? Request it below.",
    selected: "selected",
    ...
    requestHint:
      "Request a new icon and the super admin can design it for your activity and the shared library.",
  ...
  ar: {
    title: "مميزات النشاط",
    subtitle:
      "اختر المميزات المتاحة في النشاط. إذا لم تجد ما يناسبك يمكنك طلب أيقونة جديدة.",
    ...
    requestHint:
      "اطلب أيقونة جديدة وسيتمكن السوبر أدمن من تصميمها لهذا النشاط وللمكتبة المشتركة.",
...
export default function ActivityIconsStep() {
      ...
      const next = prev.filter(
        (reqItem) =>
          !icons.some((icon) => {
            ...
            return (
              String(icon._id) === String(reqItem.id) ||
              (normalized(icon.label) === normalized(reqItem.label) &&
                normalized(icon.category) === normalized(rc)) ||
              (reqItem.labelAr &&
                normalized(icon.labelAr) === normalized(reqItem.labelAr) &&
                normalized(icon.category) === normalized(rc))
            );
          }),
      );
  ...
  const handleNext = () => {
    sessionStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ selectedIcons: [...selected], requestedItems }),
    );
  ...
  return (
    <div className="page-shell">
      ...
      <div className="mt-8 flex justify-between">
        <button
          onClick={() => navigate("/activity/details/description")}
